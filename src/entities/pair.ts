...
const composeKey = (token0: Token, token1: Token, flag: boolean) => `${token0.chainId}-${token0.address}-${token1.address}-${flag}`

export class Pair {
  public readonly liquidityToken: Token
  private readonly tokenAmounts: [TokenAmount, TokenAmount]

  public static getAddress(tokenA: Token, tokenB: Token,  flag: boolean = true): string {
    const [token0, token1] = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA] // does safety checks

    const key = composeKey(token0, token1, flag)
    ...
    if (PAIR_ADDRESS_CACHE?.[key] === undefined) {
      PAIR_ADDRESS_CACHE = {
        ...
        [key]: getCreate2Address(
          ...
          keccak256(['bytes'], [pack(['address', 'address', 'bool'], [token0.address, token1.address, flag])]),
